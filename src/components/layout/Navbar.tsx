  ...
  return (
    <header
      className={cn(
        ...
        isTransparent 
          ? "bg-dark-slate/40 backdrop-blur-[2px] text-white py-2.5 sm:py-4 md:py-6" 
          : "bg-dark-slate/95 backdrop-blur-md shadow-md border-b border-white/10 text-white py-2 sm:py-2.5 md:py-3",
    ...
    >
      <div className="container mx-auto px-3 sm:px-6 lg:px-12 flex items-center justify-between gap-1.5 sm:gap-4 md:gap-8">
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                className={cn(
                  "font-display text-[11px] xs:text-xs sm:text-sm md:text-lg lg:text-xl tracking-wider sm:tracking-[0.1em] uppercase font-bold whitespace-nowrap transition-all py-1 px-1 sm:px-2 rounded hover:text-mustard-gold flex flex-col items-center",
                  isActive 
                    ? "text-mustard-gold" 
        ...
        {/* LANGUAGE SWITCHER */}
        <div className="flex items-center shrink-0 pl-1.5 sm:pl-2 border-l border-white/10 sm:border-transparent">
          <LanguageSwitcher />
        </div>
